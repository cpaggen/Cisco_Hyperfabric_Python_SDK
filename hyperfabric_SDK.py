...
def _make_get_request(auth, url, params=None):
    try:
...
def _make_delete_request(auth, url):
    try:
...
def _make_put_request(auth, url, payload=None):
    try:
        response = requests.put(url, headers=auth, json=payload)
        response.raise_for_status()  # Raise HTTPError for bad responses (4xx or 5xx)
...
def _make_post_request(auth, url, payload=None):
    try:
